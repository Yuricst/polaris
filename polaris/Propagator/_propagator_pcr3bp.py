...
def propagate_pcr3bp(
    ...
    if events == None and ivp_method == "LSODA" and force_solve_ivp == False:
        # use odeint
        propout, infodict = propagate_pcr3bp_odeint(
            mu,
            state0,
            tf,
            steps=steps,
        ...
        if infodict["message"] != "Integration successful." and switch_solveivp == True:
            ...
            propout = propagate_pcr3bp_solve_ivp(
                mu,
                state0,
                tf,
                steps=steps,
...
# ---------------------------------------------------------------------------------------- #
def propagate_pcr3bp_odeint(
    mu, state0, tf, steps=2000, t0=0.0, stm_option=False, ivp_rtol=1e-12, ivp_atol=1e-12
):
    ...
    # if initial STM is provided, also propagate STM (i.e. integrate 6+36=42 differential equations)
        ...
        state0_modified = np.array(
            [state0[0], state0[1], 0.0, state0[2], state0[3], 0.0]
        )
...
# ---------------------------------------------------------------------------------------- #
def propagate_pcr3bp_solve_ivp(
    mu,
    state0,
    tf,
    steps=2000,
