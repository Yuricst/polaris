...
# ---------------------------------------------------------------------------------------- #
def propagate_pcr3bp(mu, state0, tf, steps=200, t0=0.0, stm_option=False, events=None, ivp_method='LSODA', ivp_rtol=1e-12, ivp_atol=1e-12, force_solve_ivp=False, switch_solveivp=True, message=False):
...
def propagate_pcr3bp_odeint(mu, state0, tf, steps=2000, t0=0.0, stm_option=False, ivp_rtol=1e-12, ivp_atol=1e-12):
    ...
    if stm_option==False:
        ...
        # 2D propagation
        sol, infodict = odeint(func=rhs_pcr3bp, y0=state0, t=timesay, args=(mu,), Dfun=None, col_deriv=0, full_output=1, ml=None, mu=None, rtol=ivp_rtol, atol=ivp_atol, tcrit=None, h0=0.0, hmax=0.0, hmin=0.0, ixpr=0, mxstep=0, mxhnil=0, mxordn=12, mxords=5, printmessg=0, tfirst=True)
    ...
    # if initial STM is provided, also propagate STM (i.e. integrate 6+36=42 differential equations)
        ...
        sol, infodict = odeint(func=rhs_pcr3bp_with_STM, y0=state0ext, t=timesay, args=(mu,), Dfun=None, col_deriv=0, full_output=1, ml=None, mu=None, rtol=ivp_rtol, atol=ivp_atol, tcrit=None, h0=0.0, hmax=0.0, hmin=0.0, ixpr=0, mxstep=0, mxhnil=0, mxordn=12, mxords=5, printmessg=0, tfirst=True)
        # unpack cartesian state and time
        times = timesay       # time
        xs  = sol[:,0]
        ys  = sol[:,1]
...
def propagate_pcr3bp_solve_ivp(mu, state0, tf, steps=2000,t0=0.0, stm_option=False, events=None, ivp_method="LSODA", ivp_rtol=1e-12, ivp_atol=1e-12):
    ...
    if stm_option==False:
        # propagate state
        sol = solve_ivp(fun=rhs_pcr3bp, t_span=(0,tf), y0=state0, events=events, t_eval=timesay, args=(mu,), method=ivp_method, rtol=ivp_rtol, atol=ivp_atol)
        # unpack cartesian state and time
        times = sol.t       # time
        xs  = sol.y[0]
        ys  = sol.y[1]
    ...
    # if initial STM is provided, also propagate STM (i.e. integrate 6+36=42 differential equations)
        ...
        state0ext[5+36] = 1
        # propagate state and stm
        sol = solve_ivp(fun=rhs_pcr3bp_with_STM, t_span=(0,tf), y0=state0ext, events=events, t_eval=timesay, args=(mu,), method=ivp_method, rtol=ivp_rtol, atol=ivp_atol)
    ...
    # prepare output
    return Propout(xs, ys, None, vxs, vys, None, times, state0, statef, stms, dstatef, eventStates, eventTimes)
