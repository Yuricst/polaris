...
def propagate_ccr4bp(
    ...
    if events == None and ivp_method == "LSODA" and force_solve_ivp == False:
        # use odeint
        propout, infodict = propagate_ccr4bp_odeint(
            state0,
            tf,
            mu1,
            mu2,
            ...
            ivp_rtol=ivp_rtol,
            ivp_atol=ivp_atol,
            state0_center=state0_center,
            full_output_odeint=True,
        ...
        if infodict["message"] != "Integration successful." and switch_solveivp == True:
            ...
            # use solve_ivp
            propout = propagate_ccr4bp_solve_ivp(
                state0,
                tf,
                mu1,
                mu2,
...
def propagate_ccr4bp_solve_ivp(
    ...
    sol = solve_ivp(
        ...
        events=events,
        t_eval=timesay,
        args=(mu1, mu2, mu3, om2, om3, theta02, theta03, d2, d3),
        method=ivp_method,
        rtol=ivp_rtol,
        atol=ivp_atol,
    )
    ...
    try:
        dstatef = rhs_ccr4bp(
            times[-1], statef, mu1, mu2, mu3, om2, om3, theta02, theta03, d2, d3
        )
    except:
        dstatef = []
    # check for events
    ...
    if stm0 == False:
        ...
        if full_output_odeint == False:
            sol = odeint(
                ...
                Dfun=None,
                col_deriv=0,
                full_output=full_output_odeint,
                ml=None,
                mu=None,
                rtol=ivp_rtol,
                atol=ivp_atol,
