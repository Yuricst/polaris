...
# @jit(nopython=True)
def get_linearmotion_ic(
    mu, xi0, eta0, zeta0, lp=5, message=False, period_length="long"
):
    ...
    if period_length == "short":
        ...
        period = 2 * np.pi / s2
        theta_ta = (
            np.arctan(2 * Gamma2 * uxy / (Gamma2 ** 2 * (4 * s2 ** 2 + uxy ** 2) - 1))
            / 2
        )  # FIXME!
        ...
        period = 2 * np.pi / s1
        theta_ta = (
            np.arctan(2 * Gamma1 * uxy / (Gamma1 ** 2 * (4 * s1 ** 2 + uxy ** 2) - 1))
            / 2
        )  # FIXME!
...
if __name__ == "__main__":
    ...
    eta0 = 0.0
    zeta0 = 0.0
    xidot0, etadot0, period, theta_ta = get_linearmotion_ic(
        mu=mu, xi0=xi0, eta0=eta0, zeta0=zeta0, lp=4, message=True, period_length="long"
    )
    print(
